...
public class Task_02 {
    public static void main(String[] args){
        ...
        cabin[] CrShip = new cabin[12];  // Create an array of 12 cabins.

        for (int i = 0; i < CrShip.length; i++){
            CrShip[i] = new cabin("empty");
        }
            ...
            if(cabinNum >= 0 && cabinNum < 12){  //check if the suggested cabin number is inside the cabin range
                if (!A[cabinNum].isFull()){
                    ...
                    for (int p=0; p < passengerCount; p++){
                        ...
                        A[cabinNum].availability = "Occupied"; //Updating the initialized Empty status
                    }
                }else {
    ...
    public static void Delete_customer(cabin[] D){
        ...
        if (found == 0) { //if an invalid input
            System.out.println("sorry! The passenger you request is not on the list.");
        }
    }
    ...
    public static void expenses (cabin [] CabinNum) {
        ...
        System.out.println("\n\nTotal Expenses for passengers: " + totalExpenses(CabinNum));     //referring to a method for calculating total costs
    }
